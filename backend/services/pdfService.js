...
export const generateEstimatePDF = (estimate) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Format estimate number from ID (first 8 chars)
      const estimateNumber = estimate.id ? `EST-${estimate.id.slice(0, 8).toUpperCase()}` : 'EST-DRAFT';
      const createdDate = formatDate(estimate.created_at);
      const status = estimate.status ? estimate.status.toUpperCase() : 'DRAFT';
      const customerName = estimate.customer_name || 'Customer';
      const companyName = estimate.company_name || '';
      const estimateTitle = estimate.title || 'Estimate';
      ...
      // Left side - Estimate text
      doc
        .fillColor(textColor)
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('ESTIMATE', marginLeft + 20, marginLeft + 18);
      ...
      doc
        ...
        .fontSize(12)
        .fillColor(accent)
        .text(`${estimateNumber}`, rightHeaderX, marginLeft + 18, { width: 180, align: 'right' });
      ...
      doc
        ...
        .text('ESTIMATE FOR', marginLeft, yPosition);
      ...
      if (estimate.customer_email) {
        yPosition = doc.y + 5;
        doc.text(estimate.customer_email, marginLeft, yPosition, { width: 280 });
      }

      if (estimate.customer_phone) {
        yPosition = doc.y + 4;
        doc.text(estimate.customer_phone, marginLeft, yPosition, { width: 280 });
      }

      if (estimate.customer_address) {
        yPosition = doc.y + 4;
        doc.text(estimate.customer_address, marginLeft, yPosition, { width: 280 });
      }

      const leftBlockEnd = doc.y;

      // Estimate details (right column) - modern styling
      ...
      doc
        ...
        .fontSize(10)
        .fillColor(textSecondary)
        .text('ESTIMATE DETAILS', rightColumnX, rightY);
      rightY = doc.y + 10;
      ...
      doc
        ...
        .fontSize(14)
        .fillColor(textColor)
        .text(estimateTitle, marginLeft, yPosition, { width: contentWidth });
      ...
      if (typeof estimate.line_items === 'string') {
        try {
          lineItems = JSON.parse(estimate.line_items);
        } catch (e) {
          lineItems = [];
        }
      ...
      addTotalLine('Subtotal', formatMoney(estimate.subtotal || 0), false, textSecondary);
      addTotalLine('Tax', formatMoney(estimate.tax || 0), false, textSecondary);
      addTotalLine('Total', formatMoney(estimate.total || 0), true, accent);

      // Notes Section - modern styling
      if (estimate.notes) {
        yPosition += 16;
        const noteText = estimate.notes;
